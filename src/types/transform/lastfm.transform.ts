    ...
    const ret: Album = {
      ...
      images: album.image
        ? album.image.map(image => ({
            url: image['#text'],
            width: LastfmTransform.LImageSizeToWidth[image.size],
          }))
        : [],
    };

    if ((<any>album).tracks !== undefined) {
      ret['tracks'] = (<any>album).tracks.track.map(track =>
        LastfmTransform.transformTrack(track)
      );
    }
  ...
  static transformArtist(artist: LArtistMatch, albums?: LAlbumMatch[]): Artist {
    return {
      ...
      images: artist.image
        ...
        : [],
      albums: albums
        ? albums.map(album => LastfmTransform.transformAlbum(album))
        : undefined,
    };
  }
