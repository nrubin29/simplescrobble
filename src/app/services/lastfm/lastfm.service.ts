...
export class LastfmService implements MusicService {
  ...
  async searchTracks(q: string, limit: number, page: number): Promise<SearchResult<Track>> {
    const trackMatches = await this.httpClient.get<LTrackSearchResponse>(this.buildURL('track.search', {track: q, limit: limit.toString(), page: page.toString()}, ['track'])).toPromise();
    const tracks: Track[] = [];

    for (const trackMatch of trackMatches.results.trackmatches.track) {
      const track = (await this.httpClient.get<LTrackInfoResponse>(this.buildURL('track.getInfo', trackMatch.mbid ? {mbid: trackMatch.mbid} : {track: trackMatch.name, artist: trackMatch.artist}, ['track', 'artist'])).toPromise()).track;
  ...
  async searchAlbums(q: string, limit: number, page: number): Promise<SearchResult<Album>> {
    const albumMatches = await this.httpClient.get<LAlbumSearchResponse>(this.buildURL('album.search', {album: q, limit: limit.toString(), page: page.toString()}, ['album'])).toPromise();

    return {
      count: +albumMatches.results['opensearch:totalResults'],
      results: albumMatches.results.albummatches.album.map(albumMatch => LastfmTransform.transformAlbum(albumMatch))
    };
  }

  async searchArtists(q: string, limit: number, page: number): Promise<SearchResult<Artist>> {
    const artistMatches = await this.httpClient.get<LArtistSearchResponse>(this.buildURL('artist.search', {artist: q, limit: limit.toString(), page: page.toString()}, ['artist'])).toPromise();

    return {
      count: +artistMatches.results['opensearch:totalResults'],
      results: artistMatches.results.artistmatches.artist.map(artistMatch => LastfmTransform.transformArtist(artistMatch))
    };
  }
  ...
  async getUserInfo(): Promise<User> {
    if (localStorage.getItem('name')) {
      const userResponse = await this.httpClient.get<LUserResponse>(this.buildURL('user.getInfo', {user: localStorage.getItem('name')})).toPromise();
