...
export class SearchComponent implements OnInit {
  ...
  submit() {
    this.results = {};
    this.viewType =
      this.formGroup.getRawValue().type === 'track' ? 'list' : 'grid';
    this.loadData();
  }
  ...
  async loadData() {
    ...
    const searchResults = await this.backendService.search(
      this.formGroup.getRawValue().type,
      this.formGroup.getRawValue().query,
      page.pageSize,
      page.pageIndex + 1
    );
  ...
  async onPage(event: PageEvent) {
    ...
    if (
      event.pageSize === this.previousPageSize &&
      !(event.pageIndex in this.results)
    ) {
      await this.loadData();
    }
  }
